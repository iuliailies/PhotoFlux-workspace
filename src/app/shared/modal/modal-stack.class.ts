...
export class ModalStack {
  ...
  private windowComponentChanged = new Subject<void>();
  ...
  constructor(
    ...
    this.windowComponentChanged
      ...
      .subscribe(() => {
        const activeWindowComponent =
          this.windowComponents[this.windowComponents.length - 1];
  ...
  open(
    ...
    activeModal.execute = (result?: any) => {
      modalRef.execute(result);
    };
  ...
  private attachWindowComponent(
    ...
    const windowFactory =
      moduleComponentFactoryResolver.resolveComponentFactory(ModalComponent);
    const windowComponentRef = windowFactory.create(
      this.injector,
      contentRef.nodes
    );
  ...
  private createFromComponent(
    ...
    const contentCmptFactory =
      moduleComponentFactoryResolver.resolveComponentFactory(content);
  ...
  private setOpened(comp: ComponentRef<ModalComponent>): void {
    const el = comp.location.nativeElement.querySelector(
      '.modal-overlay'
    ) as HTMLElement;
  ...
  private registerWindowComponent(
    windowComponent: ComponentRef<ModalComponent>
  ): void {
