...
export class ModalRef<T = any> {
  ...
  private readonly _executeAction = new Subject<any>();
  executeAction: Observable<any> = this._executeAction.asObservable();

  // useful for setting component properties
  ...
  constructor(
    ...
    this.windowComponentRef?.instance.dismissEvent.subscribe((reason: any) =>
      this.dismiss(reason)
    );
  ...
  dismiss(reason?: any): void {
    ...
    if (!this.beforeDismiss) {
      ...
      if (dismiss && dismiss instanceof Promise) {
        dismiss.then((result) =>
          result !== false ? this.doDismiss(reason) : false
        );
  ...
  execute(result?: any): void {
    this._executeAction.next(result);
  }
